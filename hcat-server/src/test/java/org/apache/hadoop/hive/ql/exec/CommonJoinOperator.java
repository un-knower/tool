...
public abstract class CommonJoinOperator<T extends JoinDesc> extends
    ...
    protected void initializeOp(Configuration hconf) throws HiveException {
        ...
        for (Byte alias : order) {
            ...
            //FIXME
            LOG.warn("**************");
            LOG.warn("{}", /*rowContainerStandardObjectInspectors[pos]!=null ? Arrays.toString(rowContainerStandardObjectInspectors[pos].toArray()) : "null", */
                    JoinUtil.getSpillTableDesc(alias, spillTableDesc, conf, !hasFilter(pos)).getProperties().toString());
            LOG.warn("**************");
    ...
    protected void checkAndGenObject() throws HiveException {
        if (condn[0].getType() == JoinDesc.UNIQUE_JOIN) {
            ...
            for(int i = 0; i < storage.length; i++) {
                AbstractRowContainer.RowIterator<List<Object>> iter = storage[i].rowIter();
                for(List<Object> row = iter.first(); row != null; row = iter.next()) {
                    LOG.warn("row: " + Arrays.toString(row.toArray()));
                }
            }
            LOG.warn("*************");
