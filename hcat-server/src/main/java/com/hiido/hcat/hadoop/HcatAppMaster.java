...
public class HcatAppMaster extends CompositeService {

	private static final Log LOG = LogFactory.getLog(HcatAppMaster.class);
	...
	public HcatAppMaster(ApplicationAttemptId applicationAttemptId, ContainerId containerId, String nmHost, int nmPort,
			int nmHttpPort, long appSubmitTime, int maxAppAttempts) {
		this(applicationAttemptId, containerId, nmHost, nmPort, nmHttpPort, new SystemClock(), appSubmitTime,
				maxAppAttempts);
	}

	public HcatAppMaster(ApplicationAttemptId applicationAttemptId, ContainerId containerId, String nmHost, int nmPort,
			int nmHttpPort, Clock clock, long appSubmitTime, int maxAppAttempts) {
		//FIXME
	...
	// FIXME
	private void writeSplits(Configuration conf) throws IOException, InterruptedException, ClassNotFoundException {
	...
	// FIXME
	private <T extends InputSplit> int writeNewSplits(JobContext job, String path)
			throws IOException, InterruptedException, ClassNotFoundException {
		Path jobSubmitDir = new Path(path);
	...
	// FIXME
	private int writeOldSplits(JobConf job, String path) throws IOException {
		Path jobSubmitDir = new Path(path);
					...
					protected void unregister() {
					    try {
					    	FinalApplicationStatus finishState = FinalApplicationStatus.UNDEFINED;
					    	 ...
					    	 int rmPollInterval =
					    		        getConfig().getInt(MRJobConfig.MR_AM_TO_RM_HEARTBEAT_INTERVAL_MS,
			...
			dispatcher.getEventHandler()
					.handle(new JobHistoryEvent(job.getID(),
							new AMStartedEvent(info.getAppAttemptId(), info.getStartTime(), info.getContainerId(),
									...
									info.getNodeManagerHttpPort())));
		}

		// Send out an MR AM inited event for this AM.
		dispatcher.getEventHandler().handle(new JobHistoryEvent(job.getID(),
				new AMStartedEvent(amInfo.getAppAttemptId(), amInfo.getStartTime(), amInfo.getContainerId(),
						amInfo.getNodeManagerHost(), amInfo.getNodeManagerPort(), amInfo.getNodeManagerHttpPort(),
	...
	public static void main(String[] args) {
		try {
			...
			HcatAppMaster appMaster = new HcatAppMaster(applicationAttemptId, containerId, nodeHostString,
	...
	static class MRAppMasterShutdownHook implements Runnable {
		HcatAppMaster appMaster;

		MRAppMasterShutdownHook(HcatAppMaster appMaster) {
	...
	protected static void initAndStartAppMaster(final HcatAppMaster appMaster, final JobConf conf, String jobUserName)
