...
public class HiveSparkClientFactory {
    ...
    public static Map<String, String> initiateSparkConf(HiveConf hiveConf) {
        ...
        //FIXME HIVE-15104
        boolean optShuffleSerDe = hiveConf.getBoolean("hive.spark.optimize.shuffle.serde", false);
        if(!optShuffleSerDe) {
            classes.add(BytesWritable.class.getName());
            classes.add(HiveKey.class.getName());
        } else
            sparkConf.put("spark.kryo.registrator", "org.apache.hadoop.hive.ql.exec.spark.HiveKryoRegistrator");
