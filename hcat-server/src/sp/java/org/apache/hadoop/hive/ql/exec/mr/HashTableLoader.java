...
public class HashTableLoader implements org.apache.hadoop.hive.ql.exec.HashTableLoader {
    ...
    private void loadDirectly(MapJoinTableContainer[] mapJoinTables, String inputFileName)
        ...
        //FIXME
        ArrayList<Operator<? extends OperatorDesc>> parent = new ArrayList<>(directWorks);
        for(Operator<? extends OperatorDesc> directWork : directWorks) {
            if(directWork == null)
                continue;
            Operator<? extends OperatorDesc> work = directWork;
            while(work.getNumChild() > 0)
                work = work.getChildOperators().get(0);
            work.setChildOperators(Arrays.<Operator<? extends OperatorDesc>>asList(sink));
            //parent.add(work);
            parent.set(parent.indexOf(directWork), work);
        }
        sink.setParentOperators(parent);
