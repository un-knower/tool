...
public class SessionState {
    ...
    public void putIfAbsent(String job) {
    ...
    public float getCurProgress() {
        ...
        if(lastUpdateTime == 0l || currentTime - lastUpdateTime > 7000) {
            if(totalJob == 0)
                return 0.0f;
            else {
                float taskProgress = 0.0f;
                Iterator<Progress> iterator = runningJobs.values().iterator();
                if(iterator.hasNext()) {
                    taskProgress += iterator.next().progress;
                }
                jobProgress = (finished + taskProgress) / totalJob;
            }
    ...
    public void putRunningTask(String taskId) {
        this.runningJobs.put(taskId, new Progress(taskId));
    }

    public void updateRunningTask(String taskId, float p) {
        runningJobs.get(taskId).progress = p;
    }
